        ...
        try:
            ...
            if host_vars["ceph_origin"] == "repository" and not host_vars["containerized_deployment"]:
                ...
                if host_vars["ceph_repository"] == "community":
                    notario.validate(
                        host_vars, ceph_repository_community, defined_keys=True)
            ...
            if host_vars["osd_group_name"] in host_vars["group_names"]:
                ...
                if host_vars["osd_scenario"] == "collocated":
                    if not host_vars.get("osd_auto_discovery", False):
                        notario.validate(
                            host_vars, collocated_osd_scenario, defined_keys=True)

                if host_vars["osd_scenario"] == "non-collocated":
                    notario.validate(
                        host_vars, non_collocated_osd_scenario, defined_keys=True)
                ...
                if host_vars["osd_scenario"] == "lvm":
                    if host_vars.get("devices"):
                        ...
                        notario.validate(
                            host_vars, lvm_filestore_scenario, defined_keys=True)
                    elif notario_store['osd_objectstore'] == 'bluestore':
                        notario.validate(
                            host_vars, lvm_bluestore_scenario, defined_keys=True)
            ...
            result['msg'] = "\n".join(
                [s for s in (msg, reason, given) if len(s) > 0])
...
def osd_objectstore_choices(value):
    assert value in [
        'bluestore', 'filestore'], "osd_objectstore must be either 'bluestore' or 'filestore'"


def ceph_origin_choices(value):
    if not notario_store["containerized_deployment"]:
        assert value in ['repository', 'distro',
                         'local'], "ceph_origin must be either 'repository', 'distro' or 'local'"


def ceph_repository_choices(value):
    assert value in ['community', 'rhcs',
                     'dev'], "ceph_repository must be either 'community', 'rhcs' or 'dev'"
...
def validate_osd_scenarios(value):
    assert value in ["collocated", "non-collocated",
                     "lvm"], "osd_scenario must be set to 'collocated', 'non-collocated' or 'lvm'"
...
def validate_ceph_stable_release(value):
    assert value in CEPH_RELEASES, "ceph_stable_release must be set to one of the following: %s" % ", ".join(
        CEPH_RELEASES)
